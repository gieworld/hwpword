...
- **방식(무엇을 찾나)**: diff% 상위 페이지에서 비교 기준과 다른 지점. 단계별(10→전수→고난도
  ...
  후보로 분류한다. `--layout-ledger`의 `square_wrap_text_overlap`은 Square/Tight/Through 그림 box를
  3행 이상 침범한 Body 텍스트를 PDF raster 없이도 별도 후보화한다. 문자 순서와 공백을 무시하므로
  그 밖의 배치·줄바꿈은 픽셀/시각 대조로 확인한다.
