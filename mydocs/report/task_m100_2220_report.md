...
작업 중 `output/poc/task2004·task2019` 추적 파일 5개 삭제 상태를 발견해 당시에는 복원했다
(원인 스크립트 미특정, 게이트 재발 없음 — 커밋 전 status 확인으로 감시). 이후
[PR #4154](https://github.com/edwardkim/rhwp/pull/4154)에서 `/output/` 비추적 정책에 맞춰 해당 5개를
포함한 POC 10개를 의도적으로 제거했으며, active tree에는 복원하지 않는다.
