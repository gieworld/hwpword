...
/**
 * Segments that carry the user's own document data rather than engine text: anything quoted, and
 * any token that looks like a path or a document file name. Korean inside these is an allowed
 * exception — it is what the user named their file, bookmark or style.
 */
const USER_DATA = /"[^"\n]*"|'[^'\n]*'|\S*[\\/]\S*|\S+\.(?:hwp|hwpx|doc|docx|odt|rtf|pdf|txt)\b/gi;

const withoutUserData = (text: string): string => text.replace(USER_DATA, '');
...
export function toEnglishMessage(raw: string): string {
  ...
  // All the remaining Hangul is the user's own data (a file, bookmark or style name interpolated
  // into an app or OS message): keep the message, it is the only thing telling them what failed.
  if (!HANGUL.test(withoutUserData(text))) return text;

  // Otherwise the engine text itself is untranslated. Replace only the lines carrying it, so an
  // English wrapper the app added around it ("Could not open the file:") still reaches the user.
  const code = raw.match(CODE_TOKEN);
  const generic = code
    ? `The document engine reported an error. (${code[0]})`
    : 'The document engine reported an error.';
  return text
    .split('\n')
    .map((line) => (HANGUL.test(withoutUserData(line)) ? generic : line))
    .join('\n');
}
